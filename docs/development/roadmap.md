...
2. **Manual `call_tool()`** — next step: extend the client to call a specific tool (e.g. `get_market_snapshot`) with real arguments and print the structured result, proving the request reaches the existing `MarketDataProviderContract` → `TwelveDataMarketDataProvider` → `TwelveDataClient` chain unchanged.
