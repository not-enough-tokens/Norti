...
Documentación complementaria de arquitectura/decisiones (agregada por Integrante C): `docs/architecture/*.md`, `docs/decisions/*.md`, `docs/development/roadmap.md`. Setup de Supabase: `docs/deployment/supabase-setup.md`.
...
- M6 Financial Education (Integrante D) — 🔶 en progreso, no reflejado antes aquí: `EducationalTopic` + pivote `educational_topic_user` (con `completed_at`), `EducationalTopicController`, `FinancialEducationService::getLearningPath()`, seeder y tests ya existen. `FinancialEducationIntegrationService` quedó como código muerto (se dejó de llamar en el commit `7059129` porque filtraba montos exactos del `FinancialProfile` a la vista) — pendiente decidir si se elimina o se re-conecta sin ese leak
